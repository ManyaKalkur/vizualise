...
export const WS_URL= 'wss://vizualise.onrender.com/ws/run'
...
export function useTSPRun() {
  ...
  const runAll= useCallback((requests)=> {
      ...
      setResults((prev)=> {
        ...
        if (msg.type==='meta') {
          return {
            ...
            [runId]: {
              ...cur,
              possibleTours:msg.possible_tours,
              status: 'running'
            },
          }
        }
        ...
        return {
          ...
          [runId]: {
            ...cur,
            tour: msg.tour?? cur.tour,
            distance: msg.distance?? cur.distance,
            elapsedMs: msg.elapsed_ms,
    ...
    ws.onerror= (e)=> {
      console.error('WebSocket error: is the backend running on port 8000?', e)
      setRunning(false)
      setResults((prev)=> {
        const next= { ...prev }
        Object.keys(next).forEach((k)=> {
